...
@Composable
fun ResponsiveSample(onBack: () -> Unit) {
    ...
    BottomSheetScaffold(
        ...
        topBar = {
            TopAppBar(
                title = { Text(text = "Responsive Grid Cells") },
                navigationIcon = {
        ...
        VerticalGrid(
            columns = ExtendedGridCells.SimpleGridCells.Responsive { availableSize ->
                if (availableSize > 400.dp) {
                    SimpleGridCells.Fixed(4)
                } else {
                    SimpleGridCells.Adaptive(80.dp)
                }
            },
            modifier = Modifier
                .fillMaxSize()
