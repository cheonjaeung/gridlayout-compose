...
- `grid/`: The main library module.
  ...
  - `src/androidHostTest/`: Unit tests and snapshot tests.
...
- `./gradlew :grid:testAndroid`: Run all test cases.
- `./gradlew :grid:testAndroidHostTest --tests "TestClassName"`: Run specific tests.
- `./gradlew :grid:verifyPaparazziAndroidMain`: Run only snapshot tests.
- `./gradlew :grid:recordPaparazziAndroidMain`: Update and create new snapshots.
...
- `grid/src/androidHostTest/snapshots/images/`: Recorded snapshots generated by `./gradlew :grid:recordPaparazziAndroidMain`.
